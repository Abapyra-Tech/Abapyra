# 🌿 Abapyra Tech

Plataforma web voltada para a sustentabilidade tecnológica, conectando pessoas, técnicos e projetos sociais para reaproveitar eletrônicos e reduzir o lixo eletrônico.

<img width="1891" height="813" alt="image" src="https://github.com/user-attachments/assets/4fefa19a-81d5-42d0-8377-0975706f2507" />

---

## 💡 Sobre o Projeto

A **Abapyra Tech** transforma o problema do lixo eletrônico em oportunidade de acesso à tecnologia, geração de renda e educação. A plataforma conecta doadores, técnicos, ONGs e estudantes em um ecossistema voltado para impacto social e sustentabilidade.

---

## 📄 Páginas

| Página | Arquivo | Descrição |
|---|---|---|
| Início | `index.html` | Hero, serviços, impacto e pilares |
| Serviços | `servicos.html` | Reparo técnico e cadastro de técnicos |
| Doações | `doacoes.html` | Doe eletrônicos e histórias de transformação |
| Receba Doações | `receba-doacoes.html` | Cadastro de ONGs e projetos sociais |
| Educação | `educacao.html` | Cursos gratuitos de tecnologia |
| Sobre | `sobre.html` | Missão, visão e valores |

---

## 🎨 Identidade Visual

| Elemento | Valor |
|---|---|
| Cor primária | `#22c55e` — verde |
| Cor de destaque | `#f97316` — laranja |
| Fundo escuro | `#1F2937` |
| Fundo verde escuro | `#0d2318` |
| Fonte títulos | Jomhuria (Google Fonts) |
| Fonte corpo | Arial, sans-serif |

---

## ⚙️ Tecnologias

- **HTML5, CSS3, JavaScript** — estrutura, estilo e interatividade
- **Fetch API** — carregamento dinâmico de header, footer e acessibilidade
- **VLibras** — tradução para Língua Brasileira de Sinais
- **Web Speech API** — leitura de tela nativa do navegador
- **ViaCEP** — busca de endereço por CEP
- **Font Awesome** — ícones
- **Google Fonts** — tipografia
- **Vercel** — hospedagem

---

## 🚀 Como Rodar Localmente

1. Clone o repositório
2. Abra a pasta no **VS Code**
3. Instale a extensão **Live Server**
4. Clique com o botão direito no `index.html` → **Open with Live Server**

> ⚠️ Não abra os arquivos diretamente no navegador. O projeto usa `fetch()` para carregar componentes, o que requer um servidor local.

---

## <!-- ♿ Acessibilidade

- ✅ Tradução para Libras via VLibras
- ✅ Ajuste de tamanho de fonte
- ✅ Alto contraste
- ✅ Leitura de tela (Web Speech API)
- ✅ Preferências salvas no localStorage

---

## 👥 Equipe Abapyra Tech  

- Maria Eduarda  
- Kauanny dos Anjos
- Maria Luiza
- Jeferson Vinicius
- Gustavo Henrique
- Kawan Turchiai
- Kayk Junior
  ##
<div align="center">
  <h4>Contato</h4>
  </div>

   Obrigada por visitar o nosso repositório, caso queira se conectar com a gente, estamos disponíveis através do LinkedIn!

  **Maria Eduarda:**
  
<a href="https://www.linkedin.com/in/maria-alves-12bb031ab/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

  **Kauanny dos Anjos:**
  
<a href="https://www.linkedin.com/in/kauannyanjos/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

  **Jeferson Vinicius:**

<a href="https://www.linkedin.com/in/jeferson-vinicius-230aa2128/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

  **Gustavo Henrique:**

<a href="https://www.linkedin.com/in/shoggo/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

  **Kawan Turchiai:**

<a href="https://www.linkedin.com/in/kawanturchiai/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

  **Maria Luiza:**

<a href="https://www.linkedin.com/in/maria-luiza-cortez/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

  **Kayk Junior:**
  
<a href="https://www.linkedin.com/in/kayk-trindade-317176245/" target="_blank">
  <img src="https://img.shields.io/badge/LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
</a>

*© 2026 Abapyra Tech. Todos os direitos reservados.*
